...
class PlotTool:
    def __init__(self, master):
        ...
        self.show_labels_checkbutton = tk.Checkbutton(
            self.frame, text="Show Labels", variable=self.show_labels_var, command=self.plot_data
        )
        ...
        self.add_point_checkbutton = tk.Checkbutton(
            self.frame, text="Add Point", variable=self.add_point_var, command=self.set_add_point
        )
        self.add_point_checkbutton.grid(row=0, column=3, padx=5, pady=5)

        # 点移動のチェックボタン
        self.move_point_var = tk.BooleanVar(value=False)
        self.move_point_checkbutton = tk.Checkbutton(
            self.frame, text="Move Point", variable=self.move_point_var, command=self.set_move_point
        )
        self.move_point_checkbutton.grid(row=0, column=4, padx=5, pady=5)

        # 点削除のチェックボタン
        self.delete_point_var = tk.BooleanVar(value=False)
        self.delete_point_checkbutton = tk.Checkbutton(
            self.frame,
            text="Delete Point",
            variable=self.delete_point_var,
            command=self.set_delete_point,
        )
        self.delete_point_checkbutton.grid(row=0, column=5, padx=5, pady=5)

        # ラベル編集のチェックボタン
        self.edit_label_var = tk.BooleanVar(value=False)
        self.edit_label_checkbutton = tk.Checkbutton(
            self.frame,
            text="Edit a Label",
            variable=self.edit_label_var,
            command=self.set_edit_label,
        )
        self.edit_label_checkbutton.grid(row=0, column=6, padx=5, pady=5)

        # ラベル一括編集のチェックボタン
        self.calculate_speed_var = tk.BooleanVar(value=False)
        self.calculate_speed_checkbutton = tk.Checkbutton(
            self.frame,
            text="Edit Labels",
            variable=self.calculate_speed_var,
            command=self.set_edit_labels,
        )
        ...
        self.dark_mode_checkbutton = tk.Checkbutton(
            self.frame, text="Dark Mode", variable=self.dark_mode_var, command=self.toggle_dark_mode
        )
        self.dark_mode_checkbutton.grid(row=0, column=11, padx=5, pady=5)

        self.move_selected_var = tk.BooleanVar(value=False)
        self.move_selected_checkbutton = tk.Checkbutton(
            self.frame,
            text="Move Selected Points",
            variable=self.move_selected_var,
            command=self.set_move_selected,
        )
        self.move_selected_checkbutton.grid(row=0, column=12, padx=5, pady=5)

        self.straight_line_var = tk.BooleanVar(value=False)
        self.straight_line_checkbutton = tk.Checkbutton(
            self.frame,
            text="Straight Line",
            variable=self.straight_line_var,
            command=self.set_straight_line,
        )
        ...
        tk.Label(self.options_frame, text="Initial Label Value:").grid(
            row=0, column=0, padx=5, pady=5
        )
        self.initial_label_entry = tk.Entry(
            self.options_frame, textvariable=self.initial_label_value
        )
        self.initial_label_entry.grid(row=0, column=1, padx=5, pady=5)
        self.reset_button = tk.Button(
            self.options_frame, text="reset view", command=self.reset_view
        )
        ...
        self.multiply_label_entry = tk.Entry(
            self.options_frame, textvariable=self.multiply_label_value
        )
        self.multiply_label_entry.grid(row=1, column=5, padx=5, pady=5)
        self.multiply_button = tk.Button(
            self.options_frame, text="Multiply", command=self.multiply_label
        )
    ...
    def run(self):
        ...
        try:
            result = subprocess.run(
                ["bash", self.script_dir + "/shell.sh", path],
                check=True,
                capture_output=True,
                text=True,
            )
        ...
        if self.show_labels_var.get():
            for i in range(len(self.x)):
                txt = self.ax.text(
                    self.x[i],
                    self.y[i],
                    str(int(self.ms_to_kmh(self.labels[i]))),
                    fontsize=12,
                    ha="right",
                    color=label_color,
                )
        ...
        if event.button == MouseButton.LEFT:
            if self.move_point_var.get():
                ...
                if self.edit_labels_start is None:
                    ...
                    if end is not None:
                        ...
                        new_label = tk.simpledialog.askstring(
                            "Edit Label",
                            f"Edit label for points {self.edit_labels_start} to {end}",
                            initialvalue=self.ms_to_kmh(self.labels[self.edit_labels_start]),
                        )
                        if new_label is not None:
                            ...
                            if (
                                step < 0
                                and trj_length - self.edit_labels_start + end + 1 < trj_length / 2
                            ):
                                step = 1
                ...
                if not self.selected_range_start:
                    ...
                    if (
                        self.selected_range_start is not None
                        and self.selected_range_end is not None
                    ):
                        # 選択範囲の点を取得
                        self.selected_range_points = self.get_points_in_range(
                ...
                if not self.selected_range_start:
                    ...
                    if (
                        ...
                        x0, y0 = (
                            self.x[self.selected_range_start],
                            self.y[self.selected_range_start],
                        )
    ...
    def edit_label(self, point_idx):
        # ポイントのラベルをダブルクリックで編集
        new_label = tk.simpledialog.askstring(
            "Edit Label", f"Edit label for point {point_idx}", initialvalue=self.labels[point_idx]
        )
        if new_label is not None:
